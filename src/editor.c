...
typedef void (*command_t)(uint8_t ch);
...
static bool _command_prompt_active = false;
...
static line_t *new_line_from_cache(line_t *cache, uint16_t size)
{
	...
	if (cache->next == 0)
	{
		line = (line_t*)heap_alloc(sizeof(line_t) + size - 1);
		if (line == 0)
...
static void update_cursor(void)
{
	...
	_cursor_x = _command_prompt_active ? _buffer_prompt_len : 0;
...
static void enter_command_prompt(char *prompt, buffer_command_t accept, buffer_command_t reject)
{
	_command_prompt_active = true;
...
static void cmd_quit(uint8_t ch)
{
	con_teardown();
	sys_exit(0);
}

static void cmd_insert_char(uint8_t ch)
{
	line_t *line = _cursor.line;
...
static void cmd_insert_newline(uint8_t ch)
{
...
static void cmd_backspace(uint8_t ch)
{
	if (_cursor.offset > 0)
	{
		line_t * line = _cursor.line;
...
static void cmd_move_up(uint8_t ch)
{
...
static void cmd_move_down(uint8_t ch)
{
	uint8_t line_count = get_current_line_number();
...
static void cmd_move_left(uint8_t ch)
{
	if (_cursor.offset > 0)
	{
		--_cursor.offset;
	...
	else if (!_command_prompt_active)
	{
...
static void cmd_move_right(uint8_t ch)
{
...
static void cmd_document_save_as(uint8_t ch)
{
	enter_command_prompt("Save as:", doc_save_as, 0);
}

static void cmd_accept_buffer(uint8_t ch)
{
	if (_command_prompt_active && _buffer_accept_cmd)
		_buffer_accept_cmd();
}

static void cmd_reject_buffer(uint8_t ch)
{
	if (_command_prompt_active && _buffer_reject_cmd)
		_buffer_reject_cmd();
}
...
int main(int argc, char * argv[])
{
	...
	while (true)
	{
		...
		if (!_command_prompt_active)
		{
			snprintf(buffer, 64, "%c (%04X) %d (%d, %d) %d Kb free", (key > 32 && key <= 126) ? (char)key : '.', key, key == CON_KEY_LEFT, _cursor.line->len, _cursor.line->cap, heap_free() / 1024);
